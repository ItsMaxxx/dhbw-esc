...
Die Anwendung lässt sich auf zwei Arten starten – wähle einen Weg:

**A) Mit Docker Compose** (empfohlen, keine lokale Node-Installation nötig)
...
Der Server läuft anschließend unter `http://localhost:<APP_PORT>` (Standard: `http://localhost:3000`).
...
| **Passwort-Reset** | Reset-Link per E-Mail (zeitlich begrenzter Token) |
| **Rollen-Login** | Getrennte Logins für Viewer, Jury und Admin über dieselbe Login-Seite |
...
| **Einheitliches Voting** | Viewer **und** Jury vergeben nach klassischem ESC-Schema genau 1, 2, 3, 4, 5, 6, 7, 8, 10 und 12 Punkte an zehn Sänger (eigenes Land gesperrt) |
...
| **Rate Limiting** | Login, Signup und Passwort-Reset sind gegen Brute-Force geschützt |

---

## Voraussetzungen

Für **Docker Compose** (Weg A) reicht eine laufende Docker-Installation – Node.js wird im Container installiert.
...
Für den **lokalen Betrieb** (Weg B) wird zusätzlich Node.js benötigt:
...
Außerdem wird ein **Gmail-Konto** mit aktivierter **2-Faktor-Authentifizierung** benötigt, um Verifizierungs- und Reset-Mails zu versenden (siehe [Umgebungsvariablen](#umgebungsvariablen-env)).
...
# 2a. Nur für Weg B (lokales Node): Abhängigkeiten installieren
npm install

# 3. .env-Datei anlegen (siehe nächsten Abschnitt)
```

Bei Docker Compose werden die Abhängigkeiten beim ersten `docker compose up --build` automatisch im Container installiert.

---

## Umgebungsvariablen (.env)

Im Projektstamm muss eine Datei namens `.env` angelegt werden. Sie wird **nicht** im Repository versioniert.
...
|----------|---------|-------------|
| `APP_PORT` | ja | Port, auf dem der Express-Server lauscht (Standard: `3000`) |
...
| `SESSION_SECRET` | ja | Geheimer Schlüssel zum Signieren von Sessions (mind. 32 Zeichen, zufällig). |
| `GMAIL_EMAIL` | ja | Gmail-Adresse, von der die Mails versendet werden. |
...
> **Achtung:** Die `.env` enthält sensible Zugangsdaten und darf niemals in ein öffentliches Repository eingecheckt werden (steht bereits in der `.gitignore`).
...
Das Projekt enthält `Dockerfile`, `docker-compose.yml` und `.dockerignore`. Compose bringt sowohl den App-Container als auch den Cloudflare-Tunnel hoch.

```bash
# Bauen und im Hintergrund starten
docker compose up -d --build
...
Die `.env` im Projektstamm wird automatisch eingelesen. Für Production muss zusätzlich `CLOUDFLARED_TUNNEL_TOKEN` gesetzt sein.
...
### Weg B – Lokal mit Node.js

```bash
npm start
```

Beim Start prüft die Anwendung, ob alle Pflicht-ENV-Variablen gesetzt sind. Fehlt eine, wird der Prozess mit einer Fehlermeldung beendet.

---

## Cloudflare Tunnel (Production)

Für den Produktivbetrieb wird ein [Cloudflare Tunnel](https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/) verwendet. Der Tunnel-Container baut von innen eine Verbindung zu Cloudflare auf – es ist **kein Port-Forwarding und kein eigenes SSL-Zertifikat** nötig. HTTPS und DNS übernimmt Cloudflare.
...
2. Den angezeigten Token in die `.env` als `CLOUDFLARED_TUNNEL_TOKEN` eintragen.
3. Unter **Public Hostname** des Tunnels zwei Einträge anlegen:
   ...
   `app` ist der im `docker-compose.yml` definierte Container-Name – Cloudflare spricht intern über das Docker-Netzwerk mit dem App-Container.
...
In den Tunnel-Logs sollten vier `Registered tunnel connection`-Zeilen erscheinen – dann ist der Tunnel aktiv.

### Sicherheit

- Der Tunnel-Token ist ein Geheimnis und liegt ausschließlich in `.env` (nicht versioniert). Gerät er ins Log oder einen Chat, sollte der Tunnel in Cloudflare gelöscht und neu angelegt werden.
...
|-------|--------|----------|
| **Viewer** | Selbstregistrierung + E-Mail-Verifizierung | ESC-Punkte vergeben |
...
2. **Viewer und Jury vergeben ihre Punkte** → `POST /api/vote/viewer` bzw. `POST /api/vote/jury`
   - Klassisches ESC-Schema: genau **1, 2, 3, 4, 5, 6, 7, 8, 10, 12** Punkte.
   - Jeder Punktewert und jeder Sänger darf nur einmal verwendet werden; Viewer und Jury nutzen dieselbe Validierung.
   - Eine Abstimmung für das eigene Heimatland ist gesperrt.
   - Wiederholte Abgabe überschreibt die vorherige.

3. **Admin gibt die Ergebnisse frei** → `POST /api/admin/show-results`
   - Ab diesem Moment sehen alle Nutzer die Rangliste.
...
- **Viewer-Punkte:** Die Rohstimmen der Viewer werden *pro Herkunftsland* pro Sänger aufsummiert. Die Sänger werden dann pro Land nach Höhe dieser Rohsumme sortiert; der Spitzenreiter erhält 12 Punkte, Platz 2 erhält 10, dann 8, 7, 6, 5, 4, 3, 2, 1 – analog zum echten ESC-Telefonvoting. Diese Länder-ESC-Punkte werden anschließend über alle Herkunftsländer aufsummiert.
...
│   ├── reset-password.js        # Client-JS: Passwort-Reset
...
│   ├── country-dropdowns.js     # Client-JS: Länder-Dropdowns + Landcode↔Vorwahl-Mapping
│   ├── header.js / footer.js    # Client-JS: gemeinsames Layout
│   ├── news-cards.js            # Client-JS: News-Übersichtskarten
...
├── model/
│   ├── authModel.js             # Registrierung, Login, E-Mail-Verifizierung, Passwort-Reset
...
| `GET /reset-password` | Passwort-Reset |
...
| `GET /news/<slug>` | News-Artikel (siehe `controller/app.js` für Slug-Liste) |
...
| `POST` | `/api/forgot-password` | Passwort-Reset-Mail anfordern |
| `POST` | `/api/reset-password` | Passwort über Reset-Token setzen |
...
| [express-rate-limit](https://github.com/express-rate-limit/express-rate-limit) | Rate Limiting auf Login-, Signup- und Reset-Endpunkten |
